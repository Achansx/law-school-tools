...
## EDITORIAL-REVIEW PASS (active 2026-05-30, set by maintainer; removed at re-consolidation)

The prior FINAL-PUSH / IMMEDIATE-CITE task is COMPLETE (PI-JLE-LEGAL-ED-LIT resolved at the 2026-05-29 consolidation). This block now drives a revision pass against the run-315 editorial review, saved at `research/editorial-review-run315-2026-05-30.md` (read it; it is the authoritative finding list). While this block is present it overrides the default rotation and selection heuristics.

- **Rotation — skip Harvest and Outline.** All 14 sections are `evidence_status: populated` and the abstract + appendices are drafted, so Harvest and Outline are noops. When advancing `current_phase` (dispatcher step 7), if the next phase would be `harvest` or `outline`, skip forward to `draft`. Effective rotation: **draft -> cite -> polish -> stitch -> verify -> draft ...**. Verify is NOT throttled. Scaffold-first (step 3a) is lifted.

- **Priority order — work these `PI-ER-*` items before any generic "lowest ratio / needs_work" round-robin, highest first. Each phase picks the highest-priority open item it owns.**
  1. **PI-ER-APPENDIX (Cite + Stitch).** Reconcile EVERY appendix cross-reference against the actual files in `manuscript/appendices/` (on disk: A=Input Inventory, B=AI Tooling, C=Obsidian Note Templates, D=Karpathy-Loop Per-Phase Rubric, E=Cost and Time Log, F=Technical Setup). Three defects: (a) the ~9 footnote refs to **"App. D (Correspondence Excerpts)" / "(Iteration-Story Excerpts)" / "(Build-System Correspondence and Internal Drafts)"** (notes 5,24,56,95,96,99,102,103,119) point at appendices that do not exist AND name internal-record material that must not appear in published prose per L-057/L-058 -> STRIP the cross-ref and rewrite the supported clause to stand without it (do not build a correspondence appendix); (b) **"App. C (Cost Log)"** mis-letters the cost log -> re-letter to **App. E**; (c) the ~57 refs to **"App. A (Vault Architecture and File Layout)"** name content App. A (Input Inventory) does not contain -> either re-point to the correct appendix, condense the parenthetical to what App. A actually holds, or soften to descriptive prose. Audit ALL `App. X (...)` strings, not only D. Footnote-touching work renumbers nothing (these are body+footnote text edits); a Stitch tick propagates polished->full-draft per L-035 and re-runs the integrity audit.
  2. **PI-ER-INTERNALCITE (Cite).** Remove the 3 remaining internal-pending-issue-tracker citations at footnotes **[^163], [^165], [^167]** ("the underlying record lives in this article's internal pending-issue tracker"). Either re-support from a public source, move the record to an appendix, or cut the supported sentence. rubric/cite.md criterion 5 forbids this at score 5.
  3. **PI-ER-IXTABLE (Cite/Draft).** Synthesize the Section IX master cost-and-labor table the prose promises (IX.A calls it essential to falsifiability; IX.E + [^132] say it "remains owed"). Source it from `manuscript/cost-log.jsonl` and the existing App. E raw "Cost and Time Log" (already ~45 rows). External pricing lookups (Anthropic API, Netlify free-tier/upgrade, any metered MCP) must be WebFetch-verified live -> never invent a price. Person-hours backfill: use honest ranges where the data supports them, else `[confirm-at-proof]`. Once built, update IX.E/[^132] to stop saying "owed" and point to the correct appendix letter.
  4. **PI-ER-TRIM91 (Polish).** Trim ~91 words to land under the 14,000 ceiling. Routed to **II.B** ("flat folder of more than two hundred sixty files" inventory sentence) and **II.D** (synthesis-affordance second paragraph). II.E, II.F, X.A, XI.A are PROTECTED — do not trim them.
  5. **P2/P3 items, by phase ownership:** PI-ER-TITLE (Polish/Stitch: drop "and the Future of Legal Pedagogy" from the title); PI-ER-FN159 (Stitch: [^159] is defined out of ascending order after [^151] before [^152] — renumber to a contiguous ascending slot OR add a one-line insertion-order note); PI-ER-OVERRELIANCE (Cite: add 1-2 student-overreliance learning-sciences cites at XII.B, e.g. Westhoff / Vasconcelos, verified live); PI-ER-XIVC-COMMIT (Draft/Polish: one sentence at XIV.C committing what the author would hand a colleague attempting a second instance — schemas, rubrics, prompt library, cost-log); PI-ER-BOYER (Polish: half-sentence unpacking Boyer's scholarship-of-teaching typology at XIII.C); PI-ER-SENTENCES (Polish: split the ~80-word IV.A topic-list sentence and the three-clause-nested V.B opener); PI-ER-IIF-CLOSE (Polish: name the three commitments — accuracy, single-professor judgment, personal teaching archive — in II.F's closing sentence); PI-ER-APPA-ROUTING (Cite/Polish: continue softening literal vault filenames in footnote parentheticals to descriptive nouns).
  6. **PI-ER-FRONTMATTER (handoff only).** Do NOT strip `full-draft.md`'s YAML in place — the loop needs it. At re-consolidation, generate a SEPARATE clean submission copy (`manuscript/SUBMISSION-2026-05-30.md`) carrying only venue-needed frontmatter (title, abstract length, author placeholder).

- **PROFESSOR-ONLY items — do NOT resolve unilaterally.** Casebook-ingestion (PI-001), student-work-ingestion (PI-002), the "all 198 snapshot pages professor-reviewed" provenance claim, and the X.E candidate-course name are the professor's calls. The loop MAY draft balanced conditional language and tighten the surrounding prose, but MUST NOT assert an answer. Flag each for Professor Chandler in the handoff memo.

- **Carried-over standing rules (unchanged).** Stitch propagates polished -> full-draft on the very next tick (no batching) and re-runs the Python footnote-integrity audit (zero gap/dup/orphan; every inline marker resolves; every def used) green before pushing. Secondary-source cites still get the proposition-support check (liveness + author/title/year + actually-supports); primary-law quotes still get the Midpage findInOpinion/analyzeOpinion check. Never fabricate a quotation, citation, statistic, holding, or page. Bluebook 21st; curly quotes, no em dashes in published prose; push via `git push origin HEAD:master` with rebase fallback. This block does not change rubric scoring.

- **Batch small items within a phase (efficiency).** One-phase-per-run limits a tick to ONE phase, NOT one edit. Under this pass a single tick SHOULD address ALL open `PI-ER-*` items that the active phase owns when they are independent small edits. In particular a Polish tick should close PI-ER-TITLE + PI-ER-BOYER + PI-ER-SENTENCES + PI-ER-IIF-CLOSE + PI-ER-IXTABLE-OVERAGE together in one tick (drafts+polished), and a Cite tick should batch PI-ER-APPA-ROUTING with any other open cite-owned item. Propagate the batch to full-draft on the next Stitch. Keep the never-fabricate, Bluebook, curly-quote/no-em-dash, and footnote-integrity rules.
- **Re-consolidation exit.** When every `PI-ER-*` loop-addressable item is resolved, re-freeze: regenerate `manuscript/HANDOFF-<date>.md` + the clean `SUBMISSION-<date>.md`, update `PROFESSOR_CHANDLER_HANDOFF.md` (note the editorial-review pass, the professor-only carryovers, and any confirm-at-proof items), set `gates.handoff_freeze.awaiting_human = true`, commit, and STOP. The system still does not submit to JLE and does not email the professor.
