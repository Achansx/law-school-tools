...
- **IMMEDIATE CITE TASK (highest priority; overrides section-selection until done).** The next Cite tick(s) MUST close the two open per-source-pin-cite legs of PI-JLE-LEGAL-ED-LIT — and ONLY these — until both are done: **PI-237 at §II.E footnote [^18]** (compound cite of Carnegie Report / Best Practices / Sturm & Guinier) and **PI-241 at §X.A footnote [^150]** (compound cite of Sullivan et al. / Stuckey et al. / Maranville et al.). Do PI-237 first. For each source in the compound, add a per-source PAGE pin cite:
  - Pull the supporting page from the machine-readable sources. The Carnegie *Educating Lawyers* **Summary** PDF (archive.carnegiefoundation.org/.../elibrary_pdf_632.pdf) and the *Best Practices for Legal Education* full PDF (encle.org/.../best_practices-full.pdf) are machine-readable — `curl -sL <url> -o /tmp/s.pdf && pdftotext /tmp/s.pdf - 2>/dev/null` (or a python pdf lib), find the passage supporting the proposition, and pin to its page.
  - Support is ALREADY verified for every source in these compounds — see `research/secondary-source-citation-support-audit.md`. This task adds the page locator only; do NOT re-litigate support.
  - For any source whose exact page is NOT machine-readable from here (e.g., Sturm & Guinier's exact page in 60 Vand. L. Rev. 515; any print-only pagination), append a `[confirm-at-proof: page]` marker to that source's slot in the footnote rather than guessing. **NEVER fabricate a page number.** A partial close (real pins where readable + confirm-at-proof tags where not) fully resolves PI-237/PI-241.
  - Honor the L-064 canon-anchor footnote-length allowance (60-65 words) so pins fit. Stay within Bluebook 21st pin-cite form. Mirror polished + drafts + (next Stitch) full-draft per L-035.
  When BOTH PI-237 and PI-241 carry per-source pins-or-confirm-at-proof markers, mark PI-JLE-LEGAL-ED-LIT resolved and STOP treating this as immediate.
...
- **Stitch — clear the Section-02 propagation backlog; STOP deferring it.** The polished source-of-truth for Section 02 carries edits not yet in the assembled `full-draft.md`: the II.E Bond passage ([^9] in the section file), the II.F "Adjacent tools, and what the method adds" related-work subsection ([^10]/[^11]), and the II.E legal-ed-canon anchor ([^12]). Stitch has deferred propagating these for 5+ consecutive ticks (runs 279/284/289/294/299) because it needs new global footnote insertions plus a global footnote renumber. THIS IS NO LONGER ALLOWED. On the NEXT Stitch tick, treat THAT tick as the dedicated propagation tick: insert the new global footnotes, renumber the assembled body markers and footnote definitions so `[^1]-[^N]` stays contiguous, rewrite the affected "Section X note N" cross-references, and assemble the full current Section 02 (including II.F) into `full-draft.md`. A footnote renumber IS a complete, in-scope Stitch tick under one-phase-per-run — do not defer it again. If a later polished edit creates a new, smaller backlog, propagate it on the very next Stitch rather than batching. Run the post-Stitch Python footnote-integrity audit (zero gap / dup / orphan; every inline marker resolves to a def and every def is used) and confirm green before pushing.
- **Cite — secondary-source citation-support audit (give secondary scholarly cites EXTRA scrutiny).** For every citation to a SECONDARY scholarly source (books, law-review articles, reports, meta-reviews, policy documents — everything that is NOT a primary-law case / statute / regulation / agency adjudication), liveness + correct author/title/year is necessary but NOT sufficient. The Cite phase must also confirm the source actually SUPPORTS the proposition the footnote is attached to:
  1. WebFetch (or WebSearch) the source and read enough of it — abstract, thesis statement, the relevant section, or the publisher/SSRN description — to confirm the article's characterization of what the source says/argues/finds is accurate. Where practical, add a pin cite or a short locating phrase so the support is checkable.
  2. If the source supports a NARROWER claim than the prose asserts, narrow the prose or add an accurate "cited for X, not Y" scope note (the discipline already used at the Mertz / Abrams / Bond / Studicata footnotes). Do not let an overclaim or a source-mischaracterization stand.
  3. If the source is a non-machine-readable PDF, paywalled, or otherwise not content-readable from here (e.g., the Carnegie Report and Best Practices PDFs that WebFetch returns as binary), DO NOT assert support. Tag the claim and footnote with a `confirm-against-source` note and keep a pending issue so a human confirms the paraphrase at page proof. Never fabricate support.
  4. This is the secondary-source analogue of the primary-law Midpage `findInOpinion` / `analyzeOpinion` rule above; cases keep that stronger verbatim-against-opinion check.
  Drive this as **PI-CITE-SUPPORT-AUDIT**: sweep the EXISTING secondary-source cites for proposition-support (not just newly added ones), section by section, and record per-cite status (supported / narrowed / confirm-against-source) in the section frontmatter or claim manifest.
