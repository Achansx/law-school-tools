...
- **PI-281 / finish-the-reframe (P0 — OUTRANKS PI-ER2-VAULTCOST; do this first).** Verify run-351 found the review-claim reframe incomplete. Close it: (1) **Blanket-convert EVERY remaining `reviewed static` -> `review-ready static`** across all sites (drafts + polished + full-draft) — it is accurate everywhere given the §VIII.A snapshot disclosure, so do not leave any "reviewed static site/website/artifact" in body or footnotes. (2) **Rewrite the two surviving completed-review assertions** to the capability/role framing: §XII "moves the risk from the reader's runtime to **a human reviewer at the moment of publication**" -> a build-time review *checkpoint* the architecture places before publication (do NOT assert a reviewer acted on every page); §XIV recap "and **a professor reviewed each layer before it advanced**" -> the professor is the designed gating reviewer the workflow positions before each advance (NOT that review happened on the snapshot). (3) Confirm the explicit §VIII.A disclosure remains (snapshot shipped review-ready, not yet individually professor-approved). After the Stitch propagation, the body+footnotes must contain ZERO "reviewed static" and ZERO completed-review assertion; the next Verify Provenance Auditor must confirm PI-281 closed. Only then proceed to PI-ER2-VAULTCOST.
...
- **PI-ER2-VAULTCOST STITCH LEG (DO NOW — dedicated §IX propagation; do NOT defer or do a partial).** The new vault-construction §IX lives in `polished/section-09-cost-and-labor.md`, but `full-draft.md` STILL carries the OLD article-loop §IX (heading "Table 9.1. Article-loop ticks by phase, runs 66 to 329", total row 264 / 251,067, footnotes [^120]/[^121]/[^132]/[^133] referencing "the article loop" / "runs 66 to 329"). This Stitch tick MUST treat THIS as its whole job: replace full-draft's entire Section IX (from "# IX." through the line before "# X.") with the polished section-09 vault-construction version; apply the footnote restructure the polished file specifies (drop the article-loop-cost footnotes that polished dropped, remap polished local footnote refs to global, and renumber the global [^1]-[^N] sequence so it stays contiguous); then run the Python footnote-integrity audit (zero gap/dup/orphan; every inline marker resolves; every def used) and confirm green before pushing. ACCEPTANCE: after this tick, full-draft §IX shows "255" vault runs and "April 16 to May 8" and "vault-construction" ticks, and contains ZERO of "runs 66 to 329" / "251,067" / "264 ticks" / "article loop" cost residue. Mark PI-ER2-VAULTCOST stitch leg resolved only when that holds.
